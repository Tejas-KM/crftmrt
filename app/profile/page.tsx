...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    // Fetch profile info (email, phone, addresses)
    ...
    // No return value from useEffect
  }, [token, user?.email]);
    ...
    <div className="max-w-7xl mx-auto py-12 px-2 md:px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
        ...
        <Card className="p-8 rounded-2xl shadow-xl flex flex-col items-center w-full bg-white/90 border border-zinc-200">
          ...
          <div className="mb-2 text-base text-gray-500">Total Items: <span className="font-bold">{cart.itemCount}</span></div>
          <div className="mb-2 text-base text-gray-500">Total: <span className="font-bold">₹{cart.total}</span></div>
          {cart.items.length === 0 ? (
          ...
          <Link href="/cart" className="text-blue-600 hover:underline text-base block mt-2">Go to Cart</Link>
        </Card>
        {/* Address Book */}
        <Card className="p-8 rounded-2xl shadow-xl w-full bg-white/90 border border-zinc-200">
          <h2 className="text-xl font-bold mb-4 flex justify-between items-center">My Addresses
            <button className="text-blue-600 text-base font-medium" onClick={() => { setAddressEditIdx(profile.addresses.length); setAddressDraft({ name: "", phone: "", pincode: "", addressLine: "", city: "", state: "" }); }}>+ Add</button>
          </h2>
                ...
                {addressEditIdx === idx ? (
                  <form className="flex flex-col gap-2" onSubmit={async e => {
                    ...
                    <div className="flex gap-3 mt-2">
                      ...
                      <Button className="w-1/2 bg-zinc-200 text-zinc-800 hover:bg-zinc-300" type="button" onClick={() => setAddressEditIdx(null)}>Cancel</Button>
                    </div>
                  </form>
                ) : (
                  <div className="flex flex-col gap-1">
                    <div className="font-semibold">{addr.name} <span className="text-xs text-gray-500">({addr.phone})</span></div>
                    <div className="text-sm">{addr.addressLine}, {addr.city}, {addr.state} - {addr.pincode}</div>
                    <div className="flex gap-2 mt-1">
                      <button className="text-blue-600 text-xs font-medium" onClick={() => { setAddressEditIdx(idx); setAddressDraft(addr); }}>Edit</button>
